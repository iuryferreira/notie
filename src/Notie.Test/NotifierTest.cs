...
public class NotifierTest
{
    ...
    [Fact(DisplayName =
        "01 - AddNotification must throw NotificationIsNullException if the notification to add is null")]
    ...
    public void Clear_ShouldRemoveAllNotifications ()
    {
        ...
        {
            new Notification("any_key", "any_message"),
            new Notification("any_key", "any_message")
        };
    ...
    [Fact(DisplayName =
        "02 - AddNotifications must throw NotificationIsNullException if the list of notifications to add is null")]
    ...
    {
        _sut = new Notifier();
        _sut.AddNotification(new Notification("any_key", "any_message"));
        List<Notification> notifications = new()
        {
            new Notification("any_key", "any_message"),
            new Notification("any_key", "any_message")
    ...
    [Fact(DisplayName =
        "05 - AddNotificationsByFluent must throw ValidationResultIsNullException if the ValidationResult to add is null")]
    ...
    [Fact(DisplayName =
        "07 - AddNotificationsByFluent should overwrite the previous list if overwrite parameter is true")]
    ...
    {
        _sut = new Notifier();
        _sut.AddNotification(new Notification("key", "value"));
    ...
    private class Validator : AbstractValidator<ExampleEntity>
    {
        public Validator ()
        {
            RuleFor(_ => ExampleEntity.Field).NotEmpty();
        }
    }
